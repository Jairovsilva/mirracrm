...
export function WhatsAppView() {
  ...
  const loadAccount =
    useCallback(async () => {
      ...
      const {
        ...
        .select('id,scope_key,waba_id,phone_number_id,display_phone_number,verified_name,status')
        .eq(
          'scope_key',
  ...
  const loadSellers = useCallback(async () => {
    ...
    try {
      const result = await apiFetch('/api/whatsapp/sellers');
      setSellers((result.sellers || []) as AssignedProfile[]);
    } catch (err: any) {
    ...
    useCallback(
      async (
        ...
        try {
          ...
          if (sequence === messageSequenceRef.current && selectedIdRef.current === conversationId) {
            setMessages([]);
            if (!silent) setError(requestError?.message || 'Erro ao carregar mensagens.');
          }
        } finally {
          if (!silent && sequence === messageSequenceRef.current) setLoadingMessages(false);
        }
      },
      [apiFetch]
    );
  ...
  const handleAssign = useCallback(async (assignedUserId: string) => {
    ...
    setError(null);
    try {
      await apiFetch(`/api/whatsapp/conversations/${encodeURIComponent(id)}/assign`, {
        method: 'PATCH',
        body: JSON.stringify({ assignedUserId: assignedUserId || null }),
      });
      await loadConversations(true);
    } catch (err: any) {
      setError(err?.message || 'Não foi possível atribuir a conversa.');
    } finally {
      setAssigning(false);
    }
                ...
                <div className="flex items-center gap-2 min-w-0">
                  {isManager && (
                    <label className="flex flex-col gap-1 text-xs min-w-0">
                      <span className="text-muted-foreground">Distribuir conversa</span>
                      <select
                        aria-label="Responsável pela conversa"
                        className="max-w-[190px] rounded-md border border-input bg-background px-2 py-2 text-xs"
                        value={selectedConversation.assigned_user_id || ''}
                        disabled={assigning || loadingSellers}
