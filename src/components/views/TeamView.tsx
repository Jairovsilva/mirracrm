...
export function TeamView() {
  ...
  const getCompanyMembers = useCRMStore((s) => s.getCompanyMembers);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-border pb-4 gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {companyUsers.length} {companyUsers.length === 1 ? 'membro' : 'membros'} no workspace <span className="font-semibold text-primary uppercase">{currentUser?.companyName}</span>
          </p>
        </div>
        <div className="px-3 py-1 bg-secondary text-muted-foreground rounded-full text-xs font-medium">
          Sua função: <span className="font-bold text-primary capitalize">{currentUser?.role}</span>
        </div>
      </div>
          ...
          {companyUsers.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {companyUsers.map((user) => {
                ...
                const userLeads = leads.filter((l) => l.createdByUserId === user.id);
                  ...
                  <Card key={user.id} className="hover:shadow-md transition-shadow bg-card border-border/80 relative">
                    <CardContent className="p-5 flex flex-col justify-between h-full space-y-4">
                      ...
                      <div className="mt-auto pt-3 border-t border-border flex items-center justify-between text-xs">
                        <div>
                          <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-tight">Leads Criados</p>
                          <p className="font-bold text-sm text-foreground mt-0.5">{userLeads.length}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] text-muted-foreground font-medium uppercase tracking-tight">Empresa</p>
                          <p className="font-bold text-xs text-foreground uppercase mt-0.5">{user.companyName}</p>
                        </div>
                      </div>
